// Fluid Simulation Particle Reordering Compute Shader
...
// Add the uniform buffer for simulation parameters
struct FluidParams {
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Get the index from sorted indices array
    let sorted_index = sorted_indices[index];
    
    // Copy particle data to target array using the sorted index
    target_particles[index] = particles[sorted_index];
} 